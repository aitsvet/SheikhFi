// Treasury screen — status numbers + revenue history.
// Visible to everyone; owner sees a "Distribute" panel inline.

const { useState: useStateTr } = React;

function DistributePanel() {
  const { proposals, distributeRevenue } = useStore();
  const [sel, setSel] = useStateTr('');
  const [tx, setTx]   = useStateTr({ msg: '', tone: '' });
  const eligible = proposals
    .map((p, i) => ({ ...p, _id: i }))
    .filter(p => p.revenueReceived > p.revenuePaid);

  const doDistribute = () => {
    distributeRevenue(Number(sel));
    setTx({ msg: `Revenue distributed for proposal #${sel}.`, tone: 'ok' });
    setSel('');
  };
        ...
        <div className="field-row">
          <Field label="Proposal">
            <Select value={sel} onChange={e => setSel(e.target.value)}
              disabled={!eligible.length} style={{ width: 320 }}>
          ...
          <Button variant="primary" onClick={doDistribute} disabled={sel === ''}>
            Distribute profits
          </Button>
        </div>
...
function TreasuryScreen() {
  ...
  const settledRevenue = proposals.reduce((s, p) => s + p.revenuePaid, 0n);
  ...
  return (
    <>
      ...
      <div className="grid-2">
        <Card>
          <CardHead title="Contract" sub="Polygon Amoy" />
          <div className="card-body">
            <table className="table" style={{ fontSize: 13 }}>
              <tbody>
                <tr><th style={{ width: 160 }}>Address</th>
                  <td>
                    <a className="mono" href={`https://amoy.polygonscan.com/address/${deployment.contractAddress}`}
        ...
        <Card>
          ...
          <div className="card-body">
            <table className="table">
              ...
              <tbody>
                {proposals.filter(p => p.revenueReceived > 0n).map((p, i) => (
                  <tr key={i}>
                    <td className="num">{proposals.indexOf(p)}</td>
                    <td>{p.description}</td>
...
Object.assign(window, { TreasuryScreen, DistributePanel });
