...
⚠️ соответствует с оговоркой, ❌ разрыв (раздел в PLAN.md, «Волна v5»).
...
Две оговорки (обе → PLAN «Волна v5»). Во-первых, 3/1/6/1 обусловливает выход
уведомлением партнёров ("after giving his partner/s due notice") — `exit`
мгновенный, механизма notice нет (в cryptosarf он есть: notice + 48 ч).
Во-вторых, живые проекты стоят в книгах по номиналу до списания: партнёр,
выходящий между обесценением проекта и `writeOffProposal`, уносит долю по
номиналу и не несёт своей части убытка — фактически выход по face value,
который 3/1/6/2 запрещает; окно между обесценением и списанием ничем не
закрыто.
...
### 9. Предмет сделки: документы + сертификация совета — ⚠️
...
Оговорка: конструктор ставит `board = owner`, и «внешняя проверка» становится
внешней только после добровольного `setBoard`. AAOIFI GS 19 (¶6, ¶13) требует
совет "independent, competent, empowered … completely independent in its
function", а ¶12 — утверждения членов совета собранием участников, не
менеджментом. Разделение ролей ничем не форсируется — задокументированное
отклонение демо (PLAN «Волна v5»).
...
### 11. Менеджер — доверенное лицо; залог только под вердикт — ⚠️
...
investment" — гарантированного дохода контракт и не обещает (§7). Мелкая
оговорка: `slashCollateral` требует `!writtenOff` — после списания вердикт
за поздно вскрывшееся нарушение исполнить уже нечем (PLAN «Волна v5»).

### 12. Токенизация долей — ⚠️
...
Оговорка: «commencement of activity» утверждается, но не форсируется —
переводы SHFI открыты с нулевого дня, когда пул состоит из одних денег, а
до начала деятельности 5/2/1 подчиняет оборот сертификатов правилам сарфа
(номинал, спот), которых цепочный перевод с внеконтрактной оплатой
обеспечить не может. Гейт переводов до первого профинансированного
предложения — PLAN «Волна v5».
...
### 13. Распределение выручки после списания — ❌

Последовательность, навязанная самим контрактом: проект вернул часть тела и
успел сдать выручку (`receiveRevenue` требует `!writtenOff`), owner
списывает — `loss = released − principalReturned` **не учитывает
нераспределённую выручку, лежащую в контракте** (убыток завышен); затем
`distributeRevenue` (ветка `writtenOff`) выплачивает менеджеру
`profitRate%` этой выручки, хотя операция в целом убыточна, а остаток
раздаёт инвесторам как «прибыль» через `cumulativePerShare` — с owner-cut,
хотя по существу это возврат капитала, и его распределение не совпадает с
пропорцией, по которой убыток был только что разложен. До списания
распределить эту выручку нельзя (гейт «Principal outstanding»), так что
путь единственный. Исправление: выручка на балансе гасит недостачу в момент
списания (джабр аль-хасара), без фии менеджера и без owner-cut — PLAN
«Волна v5».
...
`test/Invariants.test.js` (детерминированный seed, воспроизводимо). Три из
них (I2, I3, I6 — пятью проверками) вдобавок **доказаны для всех входов**
символьным исполнением самого контракта — см. «Формальная верификация
(Halmos)» в конце файла.
...
| 8 | Выход по конструктивной оценке (⚠️ без notice, окно до списания) | SS 12 3/1/6/2, 3/1/5/9, 3/1/6/1 | `exit` из свободных средств | «exit pays out of free funds…»; I1, I2 |
| 9 | Гарар: документы + сертификация совета (⚠️ board=owner по умолчанию) | SS 31 4/1, 4/2/1; GS 19 ¶6/¶12/¶13 | `docsHash`, `certifyProposal` | describe «Sharia board (PLAN v3 §3)» |
...
| 12 | Доли торгуемы после начала деятельности (⚠️ без гейта до первого проекта) | SS 17 3/6, 5/2/16, 5/2/1 | ERC-20 SHFI, `_transferShares` | describe «Tokenized shares (PLAN v3 §1)»; I2 |
| 13 | Распределение после списания (❌ фи менеджера и owner-cut из возврата капитала) | SS 13 8/7; SS 40 3/2/1; SS 47 8/2 | `writeOffProposal` + `distributeRevenue` (ветка `writtenOff`) | — (PLAN «Волна v5») |
