...
С v7 вердикт двухфазный — окно на возражения: совет `proposeSlash` с
основанием on-chain, залог в объёме вердикта замораживается
(`pendingSlashTotal` — досрочный возврат тела в окне не выводит залог
из-под вердикта), по истечении `slashDelay` совет `executeSlash`. Отменить
может только сам совет (`cancelSlash`); у owner переопределения нет
намеренно — сторона капитала не стоит над вердиктом совета. Оба капа
пересчитываются в момент исполнения: усохшая недостача или списание внутри
окна маршрутизируют вердикт заново, берётся min(запрошено, залог, кап) —
только фактический ущерб. Задержка — конфигурация владельца:
`setSlashDelay` от 0 (мгновенные вердикты, как до v7) до 30 дней.
Пропорциональность восстановления доказана символьно
(`check_slashRestoresProRata`, мутационно проверено).
...
Три инварианта (I2, I3, I6) плюс неттинг списания (v5 §1) и
пропорциональность восстановления при пост-мортем вердикте (v7) вдобавок
**доказаны для всех входов** — семь символьных проверок самого контракта;
...
| 11 | Yad amanah; залог — под двухфазный вердикт с окном возражений | SS 13 разд. 6, 8/7; SS 5 2/2/1 | убыток на капитал; `proposeSlash`/`cancelSlash`/`executeSlash`, `pendingSlashTotal`-заморозка, `setSlashDelay` (конфиг 0–30 дней) | describe «Collateral (PLAN v3 §5)», «Волна v7»; Halmos `check_slashRestoresProRata` |
...
```
docker compose run --rm halmos      # 7 passed
```
...
| `check_slashRestoresProRata` | **SS 13 разд. 6; SS 5 6/8/2** | при **любом** принятом вердикте после списания восстановление увеличивает долю каждого партнёра ровно на `доля × вердикт / капитал` (после капов момента исполнения); заморозка полностью снята |
...
| восстановление делит вердикт поровну (`amount / investorAddresses.length`) вместо пропорции | `check_slashRestoresProRata` **FAIL** (20 путей), остальные шесть зелёные |
