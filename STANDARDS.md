...
`"Board is owner"`, пока `setBoard` не назначит независимый адрес — без
реального разделения сертификация невозможна. Остаётся отклонение по
процедуре назначения: AAOIFI GS 19 ¶12 требует утверждения членов совета
собранием участников, здесь совет назначает owner (для демо —
задокументированное отклонение; полный вариант — выбор советa голосованием
партнёров — в очереди PLAN).
...
них (I2, I3, I6) плюс неттинг списания (v5 §1) вдобавок **доказаны для всех
входов** — шесть символьных проверок самого контракта; см. «Формальная
верификация (Halmos)» в конце файла.
...
| 9 | Гарар: документы + сертификация совета (⚠️ назначение совета — owner, не собрание) | SS 31 4/1, 4/2/1; GS 19 ¶6/¶12/¶13 | `docsHash`, `certifyProposal` (`Board is owner` до разделения ролей) | describe «Sharia board (PLAN v3 §3)»; «certification is impossible while the board is the owner» |
