...
Текущий деплой (контракт первой версии; ре-деплой v3 — по PLAN.md §7, ждёт
секретов деплойера):
...
Веб-приложение определяет возможности контракта по ABI (feature-detect),
поэтому одинаково работает и со старым деплоем, и с v3.
